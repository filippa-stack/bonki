  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="px-6 pb-12">
        <div className="space-y-3">
          {cards.map((card, index) => <EditableCard key={card.id} card={card} index={index} onNavigate={() => navigate(`/card/${card.id}`)} onUpdate={updateCard} />)}
        </div>
      </div>
    </div>;
}
    ...
    delay: index * 0.1
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 space-y-2">
          <input type="text" value={title} onChange={handleTitleChange} onClick={handleInputClick} className="w-full font-serif text-xl text-foreground bg-transparent border-b border-transparent hover:border-border focus:border-primary focus:outline-none transition-colors placeholder:text-muted-foreground/50" placeholder="Mitt sätt - Ditt sätt" />
        ...
        <button onClick={onNavigate} className="p-2 rounded-full hover:bg-muted transition-colors" aria-label="Öppna kort">
