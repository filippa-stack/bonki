...
-- Drop the function first (it depends on the type)
DROP FUNCTION IF EXISTS public.complete_couple_session_step(uuid, integer);

-- Drop the composite type
DROP TYPE IF EXISTS public.step_completion_result;

-- Recreate function using RETURNS TABLE instead of composite type
...
begin
  ...
  v_is_step_complete := v_step_completions >= v_member_count;
  v_is_waiting := not v_is_step_complete;
  ...
  v_is_session_complete := v_is_step_complete and v_all_steps_done;
