...
begin
  -- Derive couple_space_id from the session row
  ...
  -- 1) Validate step_index belongs to session
  select exists(
  ...
  v_is_waiting := (v_step_completions > 0 and v_step_completions < v_member_count);

  -- 3) Fix session completion logic
